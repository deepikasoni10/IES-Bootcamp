...
run_step() {
  ...
  local ack_status
  # Use grep-based parsing — works everywhere without python3/jq
  if echo "$body" | grep -qiE '"status"\s*:\s*"ACK"'; then
    ack_status="ACK"
  elif echo "$body" | grep -qiE '"status"\s*:\s*"NACK"'; then
    ack_status="NACK"
  elif echo "$body" | grep -q 'subscriptions'; then
    ack_status="ACK"
  else
    ack_status="?"
  fi
